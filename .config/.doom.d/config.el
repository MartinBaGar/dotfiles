...
(setq gt-default-translator
      (gt-translator
       ...
       :engines (list (gt-google-engine :if 'word)
                      (gt-deepl-engine :if 'no-word :key "8e5f7835-fc8c-49fa-96e9-7d6e35455a2a:fx"))
       :render  (list
                      ;; (gt-posframe-pop-render :if 'selection)
                      ;; (gt-overlay-render :if 'selection :type 'replace)
                      (gt-render :if 'selection)
