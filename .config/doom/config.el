...
(with-eval-after-load 'org
  ...
  ;; 1. Wakes up the Python engine immediately when Org-mode starts
  (require 'ob-async)
  (require 'ob-python)

  ;; 2. Teach Org's internal linter that :async is a valid header argument
  (add-to-list 'org-babel-header-arg-names 'async)

  ;; 3. Setup the fastplot language aliases and Tree-sitter
  (defalias 'org-babel-execute:fastplot 'org-babel-execute:python)
  (defalias 'org-babel-prep-session:fastplot 'org-babel-prep-session:python)
  (add-to-list 'org-src-lang-modes '("fastplot" . python))

  ;; 4. The dedicated edit-buffer preparation for remote paths
  (defun org-babel-edit-prep:fastplot (info)
    (let* ((params (nth 2 info))
           (session (cdr (assq :session params)))
           (python-cmd (cdr (assq :python params))))
      (when (and session (not (string= session "none")))
        (setq-local python-shell-buffer-name session))
      (when python-cmd
        (setq-local python-shell-interpreter python-cmd))))
  )
